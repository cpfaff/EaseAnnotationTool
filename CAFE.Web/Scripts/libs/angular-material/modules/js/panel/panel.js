 ...
 *
 * @param config {!AnnotationObject=} Specific configuration object that may contain the
 ...
 *     practice.
 *   - `locals` - `{AnnotationObject=}`: An object containing key/value pairs. The keys
 *     will be used as names of values to inject into the controller. For
 *     example, `locals: {three: 3}` would inject `three` into the controller,
 *     with the value 3.
 *   - `resolve` - `{AnnotationObject=}`: Similar to locals, except it takes promises as
 ...
 *
 * @param {!AnnotationObject=} config Specific configuration object that may contain
 ...
 *   - `config` - `{!AnnotationObject=}`: The entire config object that was used in
 *     create.
 ...
 * @description
 *
 * AnnotationObject for configuring the position of the panel.
 *
 * @usage
 *
 * #### Centering the panel
...
function MdPanelService($rootElement, $rootScope, $injector, $window) {
  ...
  /** @private {!AnnotationObject<string, !MdPanelRef>} */
  this._trackedPanels = {};
...
/**
 * Creates a panel with the specified options.
 * @param {!AnnotationObject=} config Configuration object for the panel.
 * @returns {!MdPanelRef}
 */
